 ...
 *
 * Reads team_descriptions.csv from the repo root.
 */
...
if (!csv.trim()) {
  console.error("team_descriptions.csv is empty.");
  process.exit(1);
}
...
console.log(`Replacing config/teamDescriptions in project "${projectId}" (${dataRows} teams)…`);

let authSource;
try {
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS?.trim()) {
    authSource = await uploadViaServiceAccount(projectId, payload, "teamDescriptions");
  } else {
    authSource = await uploadViaFirebaseCli(projectId, payload, "teamDescriptions");
  }
} catch (err) {
  const msg = err?.message ?? String(err);
  if (msg.includes("invalid_grant") || msg.includes("invalid_rapt") || msg.includes("reauth")) {
